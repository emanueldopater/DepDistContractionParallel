...
# edges, weights, vweights, comm, clusters, 
# embed, verbose, land, forced, method, directed,
# split, seed, samples, result_name = parseargs()
...
# println(results)
# writedlm( result_name ,  results, ',')

using CGE
using DelimitedFiles

edges, weights, vweights, comm, clusters, embed, verbose, land, forced, method, result_name = parseargs()
distances = zeros(length(vweights))
if land != -1
